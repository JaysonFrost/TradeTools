...
describe('Dashboard layout', () => {
  ...
  it('supports built-in terminal window recording without requiring OBS', async () => {
    ...
    expect(settingsPanelSource).toContain('Длительность одного фрагмента')
    expect(settingsPanelSource).toContain('Память буфера, МиБ')
    ...
    expect(settingsPanelSource).toContain('listVideoEncoders')
    expect(settingsPanelSource).toContain('videoEncoderOptions')
    ...
    expect(preloadSource).toContain("ipcRenderer.invoke('recording:list-video-encoders'")
  ...
  it('routes the compact widget without creating a second capture controller', async () => {
    ...
    expect(widgetSource).toContain('clips.createBuffer()')
    expect(widgetSource).toContain('app.getRecordingWidgetAlwaysOnTop()')
    expect(widgetSource).toContain('app.toggleRecordingWidgetAlwaysOnTop()')
    expect(widgetSource).toContain('Сохранить последний буфер')
    ...
    expect(widgetSource).toContain('className={`shrink-0 ${compactIconClass}`}')
    expect(widgetSource).toContain('<ExternalLink size={14} className={compactIconClass}')
    expect(widgetSource).toContain('<X size={14} className={compactIconClass}')
    expect(widgetSource).toContain('<main\n      style={dragRegionStyle}')
    expect((widgetSource.match(/style=\{noDragRegionStyle\}/g) ?? [])).toHaveLength(5)
  ...
  it('reconciles background window sources with fresh settings only when main asks', async () => {
    ...
    expect(controllerSource).toContain('await session.encoder?.stop()')
  ...
  it('uses a separate draggable proxy chain order instead of next-server form fields', async () => {
    ...
    expect(panelSource).toContain('Подключить прокси')
    ...
    expect(panelSource).toContain('createProxyConnectionSummary')
    expect(panelSource).toContain('Обновить VPN bypass')
    expect(panelSource).toContain('VPN и маршрут')
    expect(panelSource).toContain('refreshVpnBypass')
  ...
  it('wires in-app updates through Electron and release metadata', async () => {
    ...
    expect(updateServiceSource).toContain('type ElectronAutoUpdater = typeof electronUpdater.autoUpdater')
    ...
    expect(updateServiceSource).toContain('updater.forceDevUpdateConfig = true')
    expect(updateServiceSource).toContain('updater.setFeedURL(githubFeed)')
