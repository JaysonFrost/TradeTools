...
describe('main app lifecycle', () => {
  ...
  it('waits for active render processes before releasing the app on normal quit', async () => {
    ...
    expect(appSource).toContain('const pendingBufferSave = recordingBufferSavePromise')
    expect(appSource).toContain('recordingControlQueue.catch(() => undefined).then')
    expect(appSource).toContain('pendingBufferSave?.catch(() => undefined) ?? Promise.resolve()')
  ...
  it('passes explicit Windows login item path and args for autostart', async () => {
    ...
    expect(source).toContain("const windowsLoginLaunchArg = '--windows-login'")
    expect(source).toContain('windowsProxyRuntimeStartupGraceMs = 8_000')
    expect(source).toContain('process.argv.includes(windowsLoginLaunchArg)')
  ...
  it('creates a compact pinnable recording widget with recording and buffer hotkeys', async () => {
    ...
    expect(source).toContain("getRecordingWidgetPlacement(electronScreen.getPrimaryDisplay(), process.platform === 'win32')")
    ...
    expect(widgetSource).toContain('show: true')
    expect(widgetSource).not.toContain('show: false')
    ...
    expect(source).toContain('globalShortcut.register(recordingBufferSaveAccelerator')
    expect(source).toContain('if (recordingBufferSavePromise) return recordingBufferSavePromise')
    expect(source).toContain('const savePromise = recordingControlQueue.then(async () => {')
    expect(source).toContain("if (recordingControlShuttingDown || !recordingControlStatus.enabled) throw new Error('Фоновая запись остановлена')")
    const bufferSaveSource = source.slice(source.indexOf('const saveLatestRecordingBuffer'), source.indexOf('let recordingToggleHotkeyBusy'))
    expect(bufferSaveSource.indexOf("if (recordingControlShuttingDown) throw new Error('Приложение завершает работу')"))
      .toBeGreaterThan(bufferSaveSource.indexOf('await settingsStore.load()'))
    const manualBufferSource = source.slice(source.indexOf('const enqueueManualBufferRender'), source.indexOf('const cancelClipRender'))
    expect(manualBufferSource.indexOf("if (recordingControlShuttingDown) throw new Error('Приложение завершает работу')"))
      .toBeGreaterThan(manualBufferSource.indexOf('await settingsStore.load()'))
    expect(source).toContain('const results = await Promise.allSettled(')
    expect(source).toContain("result.status === 'rejected'")
    expect(source).toContain("if (recordingBufferSavePromise) return 'Сохраняется буфер'")
    expect(source).toContain('if (recordingToggleHotkeyBusy) return')
    expect(source).toContain('saveLatestRecordingBuffer()')
    expect(controlSource).toContain("recordingBufferSaveAccelerator = 'CommandOrControl+Shift+F10'")
