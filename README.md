...
<table>
  ...
  <tr>
    <td valign="top">
      <ul>
        <li>Записывает клипы сделок через встроенную оптимизированную запись окна терминала без OBS.</li>
        <li>На Windows пишет через <code>ffmpeg</code>/<code>gdigrab</code> в main process; Chromium-рекордер остаётся fallback.</li>
        ...
        <li>Есть тяжёлый пресет <strong>10 минут до входа и 10 минут после выхода</strong>.</li>
...
### Длинные клипы: 10 минут до и после

В разделе `Видео` и в мастере есть пресет `10 минут до/после`. Он выставляет:

- `Секунд до входа`: `600`;
- `Секунд после выхода`: `600`;
- для встроенной записи: локальный буфер до входа `600`.

Это тяжёлый режим: готовый клип появится примерно через 10 минут после закрытия сделки, файлы будут крупнее, а обработка через ffmpeg займёт больше времени. Для OBS дополнительно выставьте `Replay Buffer` минимум `20 минут + обычная длина сделки`, иначе начало или конец сделки не попадут в replay.

Встроенная запись сначала использует оптимизированный `ffmpeg` backend. Если ffmpeg не смог захватить окно или платформа не поддерживает такой захват, TradeTools автоматически переключится на совместимый Chromium fallback. Для самых длинных клипов OBS Replay Buffer всё ещё остаётся самым предсказуемым вариантом.
